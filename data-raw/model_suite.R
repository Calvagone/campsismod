...
for (category in categories) {
  ...
  if (category != "testing") {
    for (model in list.files(path = paste0("data-raw/model_suite/", category), pattern = pattern)) {
      model_suite[[category]][[model]] <- campsismod::read.campsis(
        file = paste0("data-raw/model_suite/", category, "/", model)
      )
    }
  } else {
    ...
    for (subCategory in subCategories) {
      ...
      for (model in list.files(path = paste0("data-raw/model_suite/", category, "/", subCategory), pattern = pattern)) {
        model_suite[[category]][[subCategory]][[model]] <- campsismod::read.campsis(
          file = paste0("data-raw/model_suite/", category, "/", subCategory, "/", model)
        )
      }
    }
  }
