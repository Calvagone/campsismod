...
```{r}
model <- model %>% delete(InitialCondition(compartment = model %>% get_compartment_index("CENTRAL")))
```
...
# Or equivalenty:
model %>% contains(Compartment(model %>% get_compartment_index("CENTRAL")))
```
...
```{r}
model %>% replace(Compartment(1, name = "CENT")) %>% delete(Ode("A_CENTRAL")) %>% add(Ode("A_CENT", "-K*A_CENT"))
```
