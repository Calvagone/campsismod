...
#' @param check_min_max logical, check for min/max values when sampling the parameters, default is TRUE
...
#' @export
AutoReplicationSettings <- function(
  wishart = FALSE,
  odf = NA,
  sdf = NA,
  check_min_max = TRUE,
  ...
  return(new(
    "auto_replication_settings",
    wishart = as.logical(wishart),
    odf = as.integer(odf),
    sdf = as.integer(sdf),
    check_min_max = as.logical(check_min_max),
...
ManualReplicationSettings <- function(data) {
  ...
  assertthat::assert_that(
    all(data$REPLICATE %in% seq_len(nrow(data))),
    msg = "REPLICATE column must contain unique integers from 1 to nrow(data)"
  )
...
setMethod("show", signature = c("auto_replication_settings"), definition = function(object) {
  if (identical(object, AutoReplicationSettings())) {
    ...
    cat(sprintf(
      "Replication settings: wishart=%s, odf=%s, sdf=%s",
      as.character(object@wishart),
      as.character(object@odf),
      as.character(object@sdf)
    ))
  }
  cat("\n")
})
