...
sample_from_multivariate_normal_distribution <- function(parameters, n, settings) {
  ...
  assertthat::assert_that(
    all(varcovParameterNames %in% parameterNames),
  ...
  table <- sample_generic(
    fun = sample_from_multivariate_normal_distribution_core,
    args = list(mean = mean, varcov = varcov),
    n = n,
    minMax = minMax,
    msg = msg,
    settings = settings,
...
sample_from_inverse_chi_squared_or_wishart <- function(parameters, n, settings) {
  ...
  for (blockIndex in seq_along(blocks@list)) {
    ...
    if (length(block) == 1) {
      ...
      tmp <- sample_generic(
        fun = sample_from_inverse_chi_squared_core,
        args = list(df = df, scale = elem@value, variable = variable),
        n = n,
        minMax = minMax,
        msg = msg,
        settings = settings
      ...
      tmp <- sample_generic(
        fun = sample_from_inverse_wishart_core,
        args = list(df = df, mat = mat, allColnames = allColnames, wishartCorrection = wishartCorrection),
        n = n,
        minMax = minMax,
        msg = msg,
        settings = settings
...
sample_from_inverse_wishart_core <- function(n, df, mat, allColnames, wishartCorrection) {
  ...
  table <- seq_len(n) %>%
    purrr::map_df(
      ~ data.frame(t(as.vector(LaplacesDemon::rinvwishart(nu = df, S = mat * (df + correction)))))[, indexesToKeep]
    )
...
sample_generic <- function(fun, args, n, minMax, msg, settings, parameters = NULL) {
  ...
  table <- flag_sampled_parameter_rows(table = tempTable, minMax = minMax, settings = settings, parameters = parameters)
  ...
  while (sum(table$VALID) < n) {
    ...
    table <- dplyr::bind_rows(
      table,
      flag_sampled_parameter_rows(table = tempTable, minMax = minMax, settings = settings, parameters = parameters)
    )
  }
...
flag_sampled_parameter_rows <- function(table, minMax, settings, parameters) {
  ...
  if (settings@check_min_max) {
    ...
    indexesToDiscard <- parameterNames %>%
      purrr::map(.f = function(.x) {
  ...
  if (settings@check_pos_def && !is.null(parameters)) {
    ...
    toBeChecked <- toBeChecked %>%
      dplyr::left_join(
        check_matrix_is_positive_definite(table = toBeChecked, parameters = parameters),
        by = "REPLICATE"
      )
