...
append_parameters <- function(params1, params2) {
  ...
  if (any(check_collision_on_params)) {
    stop(paste0(
      "Model can't be appended because of duplicate parameter name(s): ",
      paste0(paramNames1[check_collision_on_params], collapse = ", ")
    ))
  }
...
append_varcov <- function(varcov1, varcov2) {
  if (length(varcov1) == 0 && length(varcov2) > 0) {
    ...
    assertthat::assert_that(
      !any(duplicated(colnames)),
      msg = "Duplicate parameter names in resulting variance-covariance matrix"
    )
...
setMethod(
  "add_rse",
  signature = c("parameters", "parameter", "numeric"),
  definition = function(object, parameter, value, ...) {
    parameter_ <- object %>%
      find(parameter)
...
setMethod("fix_omega", signature = c("parameters"), definition = function(object) {
  ...
  returned_omega_ <- purrr::accumulate(
    .x = omegas@list[2:length(omegas@list)],
    .f = function(.x, .y) {
...
read.varcov <- function(file) {
  ...
  assertthat::assert_that(
    all(rownames(matrix) == colnames(matrix)),
...
setMethod(
  "set_min_max",
  signature = c("parameters", "parameter", "numeric", "numeric"),
  definition = function(object, parameter, min, max, ...) {
    parameter_ <- object %>%
      find(parameter)
...
setMethod(
  "set_min_max",
  signature = c("parameters", "character", "numeric", "numeric"),
  definition = function(object, parameter, min, max, ...) {
    assertthat::assert_that(
      parameter %in% c("theta", "omega", "sigma"),
      msg = "Parameter must be one of: 'theta', 'omega' or 'sigma'"
    )
...
setMethod("show", signature = c("parameters"), definition = function(object) {
  cat("THETA's:\n")
  print(showUncertaintyOnParameters(
    object %>% select("theta"),
    discard_na_columns = c("min", "max", "label", "unit", "comment")
  ))
  cat("OMEGA's:\n")
  print(showUncertaintyOnParameters(
    object %>% select("omega"),
    discard_na_columns = c("min", "max", "same", "label", "comment")
  ))
  cat("SIGMA's:\n")
  print(showUncertaintyOnParameters(
    object %>% select("sigma"),
    discard_na_columns = c("min", "max", "label", "comment")
  ))
