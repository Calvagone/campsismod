...
test_that("Has/Add parameter method", {
  ...
  expect_true(thetas %>% hasParameter(new("theta", name="CL", index=as.integer(1), value=5, fix=TRUE)))
  expect_false(thetas %>% hasParameter(new("theta", name="V", index=as.integer(3), value=80, fix=TRUE)))
  
  thetas <- thetas %>% addParameter(new("theta", name="V", index=as.integer(3), value=80, fix=TRUE))
  expect_true(thetas %>% hasParameter(new("theta", name="V", index=as.integer(3), value=80, fix=TRUE)))
})
