...
test_that("Method get_uncertainty works as expected", {
  # This model has a variance-covariance matrix
  model <- model_suite$testing$other$`2cpt_zo_allo_metab_effect_on_cl`

  uncertainty <- model %>%
    get_uncertainty() %>%
    dplyr::mutate_if(.predicate = is.numeric, .funs = ~ signif(.x, digits = 2))
  ...
  expected <- tibble::tibble(name = character(0), se = numeric(0), 'rse%' = numeric(0))
  expect_equal(uncertainty, expected)
})
