...
test_that("Method 'add' on Campsis model, exceptions on parameters names", {
  ...
  expect_error(
    model1 %>% add(model2),
    regexp = "Model can't be appended because of duplicate parameter name\\(s\\): EPS_PROP"
  )
  ...
  expect_error(
    model1 %>% add(model2),
    regexp = "Model can't be appended because of duplicate parameter name\\(s\\): THETA_K, THETA_V, ETA_K, ETA_V, EPS_PROP"
  )
...
test_that("Method 'add_rse' works as expected", {
  ...
  expect_equal(
    tibble::tibble(name = c("THETA_CL", "THETA_Q", "OMEGA_KA"), se = c(0.5, 0.4, 0.0125), `rse%` = c(10, 10, 50)),
    uncertainty
  )
})
