...
test_that("Warnings displayed when model contains unknown statements (1)", {
  ...
  expect_warning(read.campsis(paste0(TEST_FOLDER, "custom/", modelName)), regexp="Model code contains unknown statements")
})
...
test_that("Warnings displayed when no theta, omega and sigma.csv files", {
  modelName <- "unknown_statement_hello"
  expect_warning(read.campsis(paste0(TEST_FOLDER, "custom/", modelName)), regexp="No file 'theta.csv' could be found")
  expect_warning(read.campsis(paste0(TEST_FOLDER, "custom/", modelName)), regexp="No file 'omega.csv' could be found")
  expect_warning(read.campsis(paste0(TEST_FOLDER, "custom/", modelName)), regexp="No file 'sigma.csv' could be found")
  
  model <- suppressWarnings(read.campsis(paste0(TEST_FOLDER, "custom/", modelName)))
...
test_that("Error displayed if model file has ODE's in non ODE record", {
  ...
  expect_error(suppressWarnings(read.campsis(paste0(TEST_FOLDER, "custom/", modelName))), regexp="ODE detected in non ODE record")
})

test_that("Error displayed if model file has an IF-statement in a properties record", {
  modelName <- "if_in_properties_record"
  expect_error(suppressWarnings(read.campsis(paste0(TEST_FOLDER, "custom/", modelName))), regexp="IF-statement detected in properties record")
})
...
test_that("Parsing a Campsis model with an incomplete final line shouldn't raise any warning", {
  ...
  model <- read.campsis(paste0(TEST_FOLDER, "custom/", modelName))
