...
test_that("Write/Read THETA's", {
  file <- file.path(TEST_FOLDER, "write", "parameters")
  ...
  # Write THETA's
  thetas2 <- read.parameters(file.path(file, "theta.csv"), type = "theta")
...
test_that("Disable method (IOV)", {
  model <- read.campsis(file.path(TEST_FOLDER, "custom", "model1_omega_fixed"))
...
test_that("Fix omega method is working", {
  # 'Unfix' OMEGA matrix
  original_model <- read.campsis(file.path(TEST_FOLDER, "custom", "model1_omega_fixed"))
  list <- original_model@parameters@list %>%
...
test_that("Name column is optional", {
  model <- read.campsis(file.path(TEST_FOLDER, "custom", "advan1_trans1_no_name"))
...
test_that("Parameters encoded with locale 'French' (semi-colon used as delimiter in CSV file) can be read properly", {
  ...
  model <- read.campsis(file.path(TEST_FOLDER, "custom", "csv_locale_french"))
